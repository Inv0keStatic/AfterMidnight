...
void eventlogs::paint_traverse()
{
	...
	for (unsigned int i = 0; i < logs.size(); i++) {
		...
		render::get().rect_filled(log.x + 0, last_y + log.y + 35, name_size + 10, 2, Color(126, 131, 219));
		render::get().text(font, log.x + 7, last_y + log.y + 19, Color(g_cfg.misc.log_color), HFONT_CENTERED_NONE, text);
...
void eventlogs::add(std::string text, bool full_display)
{
...
#if RELEASE
#if BETA
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ LIVE ] ")); 
#else
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ LIVE ] "));
#endif
#else
	...
	if (g_cfg.misc.log_output[EVENTLOG_OUTPUT_CHAT])
	{
		...
		auto log = crypt_str("[ \x0C LIVE \x01] ") + text;
