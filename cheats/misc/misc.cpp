...
void misc::watermark()
{
	...
	{
		if (BETA)
			name_cheat = crypt_str("LunaClient [beta]");
		else
			name_cheat = crypt_str("LunaClient");
	}
	else
	...
	render::get().rect_filled(width - 10 - box_width, 10, box_width, 2, g_cfg.menu.watermark_color);
	render::get().rect_filled(width - 10 - box_width, 12, box_width, 18, g_cfg.menu.watermark_color_background);
...
void misc::ChatSpamer()
{
	...
	{ 
		crypt_str("LunaClient.Live, Hi im a loser."),
	};
...
void misc::anti_anti_defensive(CUserCmd* m_pcmd) // Does not work. i will fix it
{
	...
	if (g_cfg.ragebot.secret && g_cfg.ragebot.anti_anti_defensive && g_cfg.ragebot.enable && g_ctx.globals.current_weapon != -1 && !started_peeking && velocity >= 5.0f)
	{
...
bool misc::teleport(CUserCmd* m_pcmd)
{
	...
	if (g_cfg.ragebot.disable_with_knife && weapon_knife)
		remove_knife = true;
...
bool misc::double_tap(CUserCmd* m_pcmd)
{
	...
	if (g_cfg.ragebot.double_tap && g_cfg.ragebot.twoshot)
	{
		for (auto i = 1; i < m_globals()->m_maxclients; i++)
		{
			auto e = static_cast<player_t*>(m_entitylist()->GetClientEntity(i));
...
void misc::hide_shots(CUserCmd* m_pcmd, bool should_work)
{
	...
	g_ctx.globals.next_tickbase_shift = m_gamerules()->m_bIsValveDS() ? 1 : 9;
	auto weapon_shoot = m_pcmd->m_buttons & IN_ATTACK || m_pcmd->m_buttons & IN_ATTACK2 && g_ctx.globals.weapon->is_knife() && g_ctx.globals.weapon->m_iItemDefinitionIndex() != WEAPON_REVOLVER;
