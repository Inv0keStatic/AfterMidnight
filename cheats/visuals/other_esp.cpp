...
void otheresp::automatic_peek_indicator()
{
	...
	if (!weapon->is_non_aim() && key_binds::get().get_key_bind_state(18) || alpha)
	{
		...
		render::get().Draw3DFilledCircle(position, 25.0f, g_ctx.globals.fired_shot ? Color(30, 240, 30, (int)(alpha * 255.0f)) : Color(g_cfg.misc.automatic_peek_color));
		...
		if (math::world_to_screen(position, screen))
		{
			...
			if (!g_ctx.globals.fired_shot)
			{
				...
				render::get().rect_filled(screen.x - 10.0f, screen.y - offset - 59.0f, 20.0f, 45.0f, Color(g_cfg.misc.automatic_peek_color));
				render::get().triangle(Vector2D(screen.x, screen.y - offset), Vector2D(screen.x - 10.5f, screen.y - offset - 14.5f), Vector2D(screen.x + 9.5f, screen.y - offset - 14.5f), Color(g_cfg.misc.automatic_peek_color));
			}
		}
	}
