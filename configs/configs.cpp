...
void C_ConfigManager::setup()
{
	...
	setup_item(&g_cfg.ragebot.dt_mode, 3, crypt_str("Exploits.doubletap_mode"));
	...
	setup_item(&g_cfg.ragebot.disable_with_knife, false, crypt_str("Exploits.disable_knife"));
	...
	setup_item(&g_cfg.misc.automatic_peek_color, Color(126, 131, 219, 200), crypt_str("Misc.quick_peek_assist_color"));
	...
	setup_item(&g_cfg.menu.watermark_color, Color(126, 131, 219), crypt_str("Menu.watermark_color"));
...
void C_ConfigManager::save(std::string config)
{
	...
	auto get_dir = [&folder, &file, &config]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
		{
			...
			file = std::string(path) + crypt_str("\\LUNA\\Configs\\") + config;
		}
